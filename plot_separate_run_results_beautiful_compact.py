...
def compact_dataset_tag(dataset: object) -> str:
    """Return a concise but recognizable dataset tag for plot filenames."""
    value = sanitize(dataset).lower()
    aliases = {
        "physionet2012": "physionet12",
        "physionet_2012": "physionet12",
        "fashionmnist": "fmnist",
...
def comparison_filename_tag(name: object) -> str:
    """Shorten known comparison names without making them ambiguous."""
    value = sanitize(name).lower()
    aliases = {
        "adaptive_vs_benchmarks": "adaptbench",
        "adaptive_vs_ablation": "adaptabl",
    }
    return aliases.get(value, value)
...
def build_plot_filename(
    metric: object,
    comparison_name: object,
    tags: Dict[str, str],
) -> str:
    """Build a compact, self-explanatory filename for a standalone plot."""
    dataset = compact_dataset_tag(tags.get("dataset", "dataset"))
    comparison = comparison_filename_tag(comparison_name)
    alpha = sanitize(tags.get("alpha", "na"))
    dp_noise = sanitize(tags.get("base_noise", "na"))
    ...
    attack_type = sanitize(tags.get("attack_type", "none"))
    malicious_ratio = sanitize(tags.get("malicious_ratio", "0"))
    if attack_type != "none" or malicious_ratio not in {"0", "0_0"}:
        parts.extend([attack_type, f"mal{malicious_ratio}"])

    return "_".join(parts)


def safe_output_path(path: str | Path, max_absolute_length: int = 250) -> Path:
    """
    Return a writable output path whose absolute name is safe on Windows.

    The parent directory is created automatically. If the absolute path is too
    long, only the filename stem is shortened and an eight-character hash is
    appended to keep filenames unique.
    """
    path = Path(path)
    path.parent.mkdir(parents=True, exist_ok=True)

    absolute_path = path.resolve()
    if len(str(absolute_path)) <= max_absolute_length:
        return path
    ...
    available_stem_length = (
        ...
        - 1  # path separator
        - len(path.suffix)
        - 1  # underscore before hash
        - len(digest)
    )

    if available_stem_length < 8:
        shortened_stem = digest
    else:
        shortened_stem = path.stem[:available_stem_length].rstrip("_- ")
        shortened_stem = f"{shortened_stem}_{digest}"

    shortened_path = path.with_name(f"{shortened_stem}{path.suffix}")
    print(
        "Warning: output path was too long and has been shortened:\n"
        f"  Original ({len(str(absolute_path))} chars): {absolute_path}\n"
        f"  New      ({len(str(shortened_path.resolve()))} chars): "
        f"{shortened_path.resolve()}"
    )
    return shortened_path


def save_figure(fig: plt.Figure, out_path: str | Path) -> None:
    """Save a figure as PDF and PNG using Windows-safe output paths."""
    out_path = Path(out_path)
    pdf_path = safe_output_path(out_path.with_suffix(".pdf"))
    png_path = safe_output_path(out_path.with_suffix(".png"))
...
def run_comparison_block(
    ...
    for metric in available_metrics(df, metrics):
        # Keep the filename concise, but retain the dataset, comparison type,
        # alpha, DP-noise, attack, and malicious ratio so a shared file remains
        # self-explanatory even when it is removed from its output folder.
        filename = build_plot_filename(metric, name, tags)
        ...
        if not summary.empty:
            ...
            if (not no_final_bars) and (metric not in set(skip_bar_metrics)):
                plot_final_bar(
                    summary=summary,
                    metric=metric,
                    out_path=dataset_dir / f"{filename}_bar",
                    use_smoothed=True,
                )
